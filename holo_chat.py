...
class HologramCreator:
    ...
    def cycle_hologram_image(self):
        ...
        # Schedule next cycle
        self.image_cycle_job = self.root.after(250, self.cycle_hologram_image)  # 1/4 second
    
    def start_image_cycling(self):
        """Start the image cycling process"""
        if self.cycle_images and len(self.cycle_images) > 1:
            self.current_image_index = 0
            self.image_cycle_job = self.root.after(250, self.cycle_hologram_image)
    ...
    def show_image_selection(self):
        ...
        tk.Label(image_frame, text="Select up to 20 images (multiple images will cycle automatically):", 
        ...
        self.images_listbox = tk.Listbox(image_frame, height=20, width=50,
                                        font=('Arial', 9))
    ...
    def select_images(self):
        ...
        if len(self.current_hologram['images']) >= 20:
            messagebox.showwarning("Limit Reached", "You can only select up to 20 images.")
            return
        ...
        for file_path in file_paths:
            if len(self.current_hologram['images']) >= 20:
                messagebox.showwarning("Limit Reached", "Maximum 20 images reached.")
                break
...
#                            command=self.show_communication_mode,
    ...
    def start_chat(self):
        ...
        self.comm_mode_var = tk.StringVar(value='voice')
